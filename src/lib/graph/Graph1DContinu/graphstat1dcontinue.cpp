...
GraphStat1DContinue::~GraphStat1DContinue() { delete ui; }
...
void GraphStat1DContinue::paintEvent() {
  ...
  unsigned int EffMin = ((Data1D&)L->getElement(0)).getHeadcount(),
               EffMax = ((Data1D&)L->getElement(0)).getHeadcount();
  ...
  while (i <= Taille - 1) {
    ...
    if (((Data1D&)L->getElement(i)).getValue() >=
        Debut + (NbIntervalle + 1) * Intervalle) {
      NbIntervalle++;
      continue;
    }
    NbIntervalle++;
  ...
  while (j < NbIntervalle) {
    ...
    painter.drawLine(
        50 + j * 400 / NbIntervalle,
        190 - ((Data1D&)L->getElement(i)).getHeadcount() * 170 / EffMax,
        50 + j * 400 / NbIntervalle, 193);
    painter.drawLine(
        50 + j * 400 / NbIntervalle,
        190 - ((Data1D&)L->getElement(i)).getHeadcount() * 170 / EffMax,
        50 + (j + 1) * 400 / NbIntervalle,
        190 - ((Data1D&)L->getElement(i)).getHeadcount() * 170 / EffMax);
  ...
  j = 1;
  while (j < NbIntervalle) {
    painter.drawLine(50 + j * 400 / NbIntervalle, 440 - D2 * 170 / EffTotal,
                     50 + j * 400 / NbIntervalle, 443);
    painter.drawLine(50 + 400 * (j - 1) / NbIntervalle,
                     440 - D1 * 170 / EffTotal, 50 + 400 * j / NbIntervalle,
                     440 - D2 * 170 / EffTotal);
    D1 = D2;
  ...
  painter.drawLine(50 + 400 * (j - 1) / NbIntervalle, 440 - D1 * 170 / EffTotal,
                   50 + 400 * j / NbIntervalle, 440 - D2 * 170 / EffTotal);
  painter.drawLine(50 + j * 400 / NbIntervalle, 440 - D2 * 170 / EffTotal,
                   50 + j * 400 / NbIntervalle, 443);
