...
DataSourceSerieDiscrete* p = dynamic_cast<DataSourceSerieDiscrete*>((E1.getSample())->getDataSource());
...
// Etendue
	...
	float Etendue = ((Data1D&)L->getElement(L->getNumberItems() - 1)).getValue() - ((Data1D&)L->getElement(0)).getValue();
	int	EffMin = ((Data1D&)L->getElement(0)).getHeadcount(), EffMax = 	((Data1D&)L->getElement(Taille - 1)).getHeadcount();
...
// Tracer l'histogramme
	for (int i = 0;i< Taille; i++)
		painter.drawRect(
50 + EcartX * (((Data1D&)L->getElement(i)).getValue() - ((Data1D&)L->getElement(0)).getValue()) ,
		    190 - ((Data1D&)L->getElement(i)).getHeadcount() * EcartY,5, ((Data1D&)L->getElement(i)).getHeadcount() * EcartY );
...
// Tracer l'histogramme cumule
	...
	painter.drawRect(50 + EcartX * (((Data1D&)L->getElement(i)).getValue() - ((Data1D&)L->getElement(0)).getValue()) ,
...
// Ecriture des valeurs 
	...
	{	sprintf(Buff,"%6.1f", ((Data1D&)L->getElement(i)).getValue());
		painter.drawText(30 + EcartX * ((Data1D&)L->getElement(i)).getValue() - ((Data1D&)L->getElement(0)).getValue(),220,Buff);  // --------------
		painter.drawText(30 + EcartX * ((Data1D&)L->getElement(i)).getValue() - ((Data1D&)L->getElement(0)).getValue(),470,Buff);
